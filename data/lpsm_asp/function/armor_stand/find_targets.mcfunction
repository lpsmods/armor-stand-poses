...
#
# Called By: lpsm_asp:change_pose

# Search for entity that was interacted with. 
scoreboard players set #bool lpsm_asp.util 0
execute on target store result score #bool lpsm_asp.util if entity @a[tag=this]
execute if score #bool lpsm_asp.util matches 1 at @s positioned ~ ~-1 ~ run tag @n[type=marker, tag=ArmorStandData, limit=1] add this
execute if score #bool lpsm_asp.util matches 1 run data remove entity @s interaction
